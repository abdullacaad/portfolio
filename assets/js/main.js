...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const projectFilterBtns = document.querySelectorAll('.filter-chip');
    ...
    function applyProjectFilters() {
        ...
        projectCards.forEach(card => {
            ...
            const searchableText = `${card.querySelector('h3')?.textContent || ''} ${card.querySelector('.project-tech')?.textContent || ''}`.toLowerCase();
        ...
        if (projectCount) {
            projectCount.textContent = `Showing ${visibleCount} project${visibleCount === 1 ? '' : 's'}`;
        }
    }

    projectFilterBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            projectFilterBtns.forEach(chip => chip.classList.remove('active'));
    ...
    if (testimonialCards.length > 1) {
        ...
        setInterval(() => {
            testimonialCards.forEach(card => card.classList.remove('active-testimonial'));
            testimonialIndex = (testimonialIndex + 1) % testimonialCards.length;
            testimonialCards[testimonialIndex].classList.add('active-testimonial');
        }, 2800);
    }
